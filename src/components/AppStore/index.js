...
class AppStore extends Component {
  constructor(props) {
    ...
    this.state = {selectedTabItem: this.tabsList[0], searchInput: ''}
  }

  updateSelectedTab = tabId => {
    const selectedTab = this.tabsList.find(tab => tab.tabId === tabId)
    this.setState({selectedTabItem: selectedTab})
  }
  ...
  render() {
    // console.log(this.tabsList, this.appsList)
    const {selectedTabItem, searchInput} = this.state
    // console.log('selected tab item', selectedTabItem)
    const filterAppsList = this.appsList.filter(
      app =>
        app.category === selectedTabItem.tabId &&
        app.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    // console.log(filterAppsList)

    return (
      <div className="app-store-outer-bg-container">
        <div className="top-section-container">
          ...
          <div className="tabs-section-bg-container">
            <ul className="tabs-section-container">
        ...
        <div className="bottom-section-container">
          <ul className="apps-bg-container">
            {filterAppsList.map(app => (
              <AppItem appItem={app} key={app.appId} />
            ))}
          </ul>
        </div>
      </div>
